...
import String exposing (concat, dropLeft, indexes, split, toInt)
...
cutToMulPosition : String -> Int -> String
cutToMulPosition s position =
    dropLeft position s
...
processPotentialMul s =
    case split "," s of
        x1 :: rest ->
            case split ")" (concat rest) of
                x2 :: _ ->
                    Maybe.withDefault 0 (toInt x1) * Maybe.withDefault 0 (toInt x2)
...
part1 input =
    input
        |> indexes "mul("
        |> map (\x -> 4 + x)
        |> map (cutToMulPosition input)
        |> map processPotentialMul
        |> sum


filterDisabledMuls : List Int -> List Int -> List Int -> List Int
filterDisabledMuls dos donts muls =
    case muls of
        [] ->
            []
        ...
        m :: mul_rest ->
            case donts of
                [] ->
                    m :: filterDisabledMuls dos donts mul_rest

                dont :: dont_rest ->
                    if dont < m then
                        -- see if we should stay swiched off or go on
                        case dos of
                            [] ->
                                -- stay swiched off forever
                                []
                            ...
                            do :: do_rest ->
                                if dont < do then
                                    if do < m then
                                        ...
                                        m :: filterDisabledMuls dos dont_rest mul_rest

                                    else
                                        -- current dont is disabling muls
                                        filterDisabledMuls dos donts mul_rest

                                else
                                    -- current dont is disabling muls, remove old dos
                                    filterDisabledMuls do_rest donts mul_rest
